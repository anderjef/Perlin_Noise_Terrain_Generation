...
public class Perlin_Noise_Terrain_Generation extends PApplet {
...
float delta = 0.1f; //how spiky the terrain is; experimentally determined to be decent
float movementSpeed = 0.02f; //how fast the terrain moves; experimentally determined to be decent (for large scale numbers, evenly divisible factors of delta produces perfect translation of points, whereas factors of large remainders produce more nuanced wavelike behavior)
...
public void setup() {
  ...
  numColumns = PApplet.parseInt(1.4f * terrainMultiplier * (width / scale)) + 1; //can be modified to increase the amount of terrain generated, but cannot be negative; multiplied relative to the number of rows to move the corners of generation outside the viewing window
  numRows = PApplet.parseInt(terrainMultiplier * (height / scale)) + 1; //can be modified to increase the amount of terrain generated, but cannot be negative
...
public void draw() {
  ...
  public void settings() {  size(600, 900, P3D); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Perlin_Noise_Terrain_Generation" };
    if (passedArgs != null) {
